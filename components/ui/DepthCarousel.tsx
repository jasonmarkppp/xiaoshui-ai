...
type DepthCarouselProps = {
  ...
  className?: string;
  /** 0–1 scroll progress; drives continuous stack position */
  scrollProgress?: number;
  /** Disable wheel/drag/autoplay — driven by ScrollTrigger */
...
export default function DepthCarousel({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current;
      const needed = cfg.cardWidth + Math.abs(cfg.spread) * 1.35 + 48;
      scaleRef.current = clamp(w / needed, 0.55, 1.15);
      layout(posRef.current);
    });
    ro.observe(root);
  ...
  }, [layout, setFocus, scrollDriven, scrollProgress]);

  useEffect(() => {
    if (scrollProgress === undefined || count < 1) return;
    tweenRef.current?.kill();
    const pos = clamp(scrollProgress, 0, 1) * Math.max(count - 1, 0);
    posRef.current = pos;
    layout(pos);
  ...
  }, [scrollProgress, count, layout, notify]);
  ...
  useEffect(() => {
    ...
    if (scrollDriven || !autoplay || reducedRef.current || count < 2) return;
    const root = rootRef.current;
    let hovered = false;
    let focused = false;
    const stop = () => {
  ...
  }, [autoplay, autoplayDelay, count, navigateBy, scrollDriven]);
  ...
  return (
    <div
      ...
      style={{
        ...rootStyle,
        cursor: scrollDriven ? "default" : undefined,
      }}
      role="group"
    ...
    >
      ...
      {showIndicators && count > 1 ? (
        <div className="depth-carousel__dots" role="tablist" aria-label="幻灯片">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
